...
export function GameCard({ game, size = "md", onClick }: GameCardProps) {
  ...
  return (
    <Card 
      className={`${sizeClasses[size]} glass-morphism border-border/50 hover:border-primary/30 transition-all duration-300 cursor-pointer group hover:scale-102`}
      onClick={onClick}
    >
      <CardHeader className="p-0">
        <div className={`${imageSizes[size]} bg-gradient-to-br from-muted/50 to-muted/30 rounded-t-lg overflow-hidden relative`}>
          <ImageWithFallback
            src={game.imageUrl}
            alt={game.title}
          ...
          {game.hasRayTracing && (
            <div className="absolute top-3 right-3">
              <Badge variant="secondary" className="text-xs bg-primary/20 text-primary border-primary/30 neon-glow">
                RTX
              </Badge>
            </div>
          )}
